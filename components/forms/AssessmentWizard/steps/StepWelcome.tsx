...
export default function StepWelcome() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="font-display text-2xl font-semibold text-wiz-ink">
      ...
      <div>
        ...
        <ol className="divide-y divide-wiz-line border-y border-wiz-line">
          {PHASES.map((phase, i) => (
            <li key={phase} className="flex items-center gap-3 py-2.5">
              <span className="font-display text-sm font-semibold tabular-nums text-wiz-accent-dark">
                {String(i + 1).padStart(2, "0")}
              </span>
              <span className="text-sm font-medium text-wiz-ink">{phase}</span>
            </li>
          ))}
        </ol>
      </div>
        ...
        <ul className="space-y-2.5">
          {RATING_LEGEND.map((l) => (
            <li key={l.rating} className="flex items-center gap-3 text-sm">
              ...
              <span className="w-20 shrink-0 font-medium text-wiz-ink">
                {l.rating === "ATTENTION" ? "Attention" : l.rating === "N/A" ? "N/A" : l.rating.charAt(0) + l.rating.slice(1).toLowerCase()}
              </span>
              <span className="text-wiz-ink/70">{l.meaning}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <button
        ...
        className="w-full rounded-lg bg-wiz-action py-3.5 text-base font-semibold text-white transition-colors hover:bg-wiz-action-dark"
      >
        Get Started
      </button>
