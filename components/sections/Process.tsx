/**
 * Process — a navy ACCENT band (one section, not a full-page surface). Four
 * connected steps with a dotted connector that the circular step markers sit on
 * top of. Navy with aqua accents.
 */
const STEPS = [
          ...
          <ol className="grid gap-10 md:grid-cols-4">
            {STEPS.map((s) => (
              <li key={s.n} className="relative flex flex-col items-center text-center">
                <span className="relative z-10 flex h-14 w-14 items-center justify-center rounded-full bg-navy font-display text-xl font-semibold text-teal ring-2 ring-teal">
                  {s.n}
                </span>
